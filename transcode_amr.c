...
static void usage(void) {
  ...
  printf("usage: transcode_amr SDPFILE CODEC PAYLOADTYPE OUTPUT [Other "
         "Options]\n");
  printf("\n"
         "Other Options:\n"
         "-h                print this help\n"
...
static int parse_payload(const char *payload_type_arg) {
  char *endptr = NULL;
  long payload_type = strtol(payload_type_arg, &endptr, 10);
  if ((payload_type == 0 && errno == EINVAL) || (payload_type_arg == endptr)) {
    fprintf(stderr, "Payload type '%s' is not a valid integer\n",
            payload_type_arg);
    exit(1);
  }
  if (payload_type > INT32_MAX) {
    fprintf(stderr, "Payload type '%s' is too big\n", payload_type_arg);
    exit(1);
  }
  return payload_type;
}
...
static int open_input_file(const char *filename,
  ...
  decode_ctx = avcodec_alloc_context3(input_codec);
  if (!decode_ctx) {
  ...
  error = avcodec_parameters_to_context(
      decode_ctx, (*input_format_context)->streams[0]->codecpar);
  if (error < 0) {
    avformat_close_input(input_format_context);
  ...
  if ((error = avcodec_open2(decode_ctx, input_codec, NULL)) < 0) {
  ...
  *input_codec_context = decode_ctx;
  return 0;
}
...
static int open_output_file(const char *filename,
                            ...
                            const AVCodec *output_codec, int sample_rate,
                            int bit_rate) {
  AVCodecContext *avctx = NULL;
  AVStream *stream = NULL;
  int error;

  avformat_alloc_output_context2(output_format_context, NULL, "rtp", filename);
  ...
  if ((error = avio_open(&(*output_format_context)->pb, filename,
                         AVIO_FLAG_WRITE)) < 0) {
  ...
  avctx->ch_layout.nb_channels = input_codec_context->ch_layout.nb_channels;
  avctx->ch_layout = input_codec_context->ch_layout;
  //   avctx->channel_layout =
  //   av_get_default_channel_layout(input_codec_context->channels);
  avctx->sample_rate = input_codec_context->sample_rate;
  // sample_rate ? sample_rate : input_codec_context->sample_rate;
  ...
  if (output_codec->id == AV_CODEC_ID_PCM_ALAW ||
      output_codec->id == AV_CODEC_ID_PCM_MULAW ||
      output_codec->id == AV_CODEC_ID_PCM_S16BE) {
...
static int init_resampler(AVCodecContext *input_codec_context,
  ...
  error = swr_alloc_set_opts2(
      ...
      input_codec_context->sample_fmt, input_codec_context->sample_rate, 0,
      NULL);
      
  if (error != 0 || !*resample_context) {
...
static int write_output_file_header(AVFormatContext *output_format_context,
                                    AVDictionary **options) {
  int error;
  if ((error = avformat_write_header(output_format_context, options)) < 0) {
...
int main(int argc, char **argv) {
  ...
  payload_type = parse_payload(payload_type_arg);
  av_dict_set(&options, "protocol_whitelist", "udp,rtp,file", 0);
  av_dict_set_int(&options, "payload_type", payload_type, 0);

  /* Open the input file for reading. */
  if (open_input_file(input_url_arg, &input_format_context,
                      &input_codec_context, &options))
    goto cleanup;

  if (verbose) {
    av_dump_format(input_format_context, 0, input_url_arg, 0);
  }

  /* Open the output file for writing. */
  if (open_output_file(output_url_arg, input_codec_context,
                       &output_format_context, &output_codec_context, codec,
                       sample_rate, bit_rate))
    goto cleanup;

  if (verbose) {
    av_dump_format(output_format_context, 0, output_url_arg, 1);
  }
  ...
  if (init_fifo(&fifo, output_codec_context))
    goto cleanup;
  /* Write the header of the output file container. */
  if (write_output_file_header(output_format_context, &options))
    goto cleanup;
