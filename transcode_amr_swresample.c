...
static int open_output_file(const char *filename,
  ...
  avctx->bit_rate = bit_rate ? bit_rate : input_codec_context->bit_rate;
...
static int encode_audio_frame(AVFrame *frame,
                              AVFormatContext *output_format_context,
                              AVCodecContext *output_codec_context,
                              int *data_present, AVFilterContext *buffersrc_ctx,
  ...
  if (init_input_frame(&filt_frame)) {
    goto cleanup;
  }
  ...
  while (1) {
    ...
    error = avcodec_send_frame(output_codec_context, filt_frame);
...
static int load_encode_and_write(AVAudioFifo *fifo,
  ...
  if (encode_audio_frame(output_frame, output_format_context,
                         output_codec_context, &data_written, buffersrc_ctx,
                         buffersink_ctx)) {
    av_frame_free(&output_frame);
    return AVERROR_EXIT;
  }
  ...
  while (1) {
    ...
    while (av_audio_fifo_size(fifo) >= output_frame_size ||
      ...
      if (load_encode_and_write(fifo, output_format_context,
                                output_codec_context, buffersrc_ctx,
                                buffersink_ctx))
        goto cleanup;
    }
